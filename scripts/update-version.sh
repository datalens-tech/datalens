...
CONTROL_API_VERSION=$(yq -r '.controlApiVersion' ./versions-config.json)
DATA_API_VERSION=$(yq -r '.dataApiVersion' ./versions-config.json)
...
echo "  control-api: ${CONTROL_API_VERSION}"
echo "  data-api: ${DATA_API_VERSION}"
echo "  ui: ${UI_VERSION}"
echo "  us: ${US_VERSION}"
...
RELEASE_VERSION="${RELEASE_VERSION}" \
  ...
  yq -i '
  .application.control_api.version = strenv(CONTROL_API_VERSION) |
...
echo ""
echo "  update docker-compose file: ./docker-compose.yml"

RELEASE_VERSION="${RELEASE_VERSION}" \
  CONTROL_API_IMAGE="ghcr.io/datalens-tech/datalens-control-api:${CONTROL_API_VERSION}" \
  DATA_API_IMAGE="ghcr.io/datalens-tech/datalens-data-api:${DATA_API_VERSION}" \
