...
resource "yandex_cm_certificate" "this" {
  ...
  domains = local.is_zitadel_enabled ? [local.domain, "zitadel.${local.domain}"] : [local.domain]

  managed {
    challenge_type  = "DNS_CNAME"
    challenge_count = local.is_zitadel_enabled ? 2 : 1
  }
}
...
locals {
  ...
  dns_records = concat(
    ...
    local.is_zitadel_enabled ? [
      {
        key  = "cert-1"
        name = yandex_cm_certificate.this.challenges[1].dns_name,
        type = yandex_cm_certificate.this.challenges[1].dns_type,
        data = yandex_cm_certificate.this.challenges[1].dns_value
      },
      {
        key  = "domain-1"
        name = "zitadel.${local.domain}.",
        type = "A",
        data = local.alb_ipv4_address
      },
  ] : [])
